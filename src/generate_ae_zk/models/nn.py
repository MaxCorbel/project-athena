...
def train(num_wds, training_data, training_labels, validation_data, validation_labels):
    ...
    print('>Training')
    history = network.fit(training_data, training_labels, epochs=5, batch_size=8)
    history.history
    
    print('>Evaluate on validation data')
    results = network.evaluate(validation_data, validation_labels, batch_size=8)
    print('>Test loss, Test acc:', results)
    
    network.save('learning-strategy-nn')
