...
def subsampling(data, labels, num_classes, ratio=0.2, filepath=None, filename=None):
    """
    Subsampling dataset.
    ...
    test_samples = np.asarray([data[i] for i in sample_ids])
    test_labels = np.asarray([labels[i] for i in sample_ids])
    
    training_samples = np.asarray([data[i] for i in range(pool_size) if i not in sample_ids])
    training_labels = np.asarray([labels[i] for i in range(pool_size) if i not in sample_ids])

    if filepath is not None:
        # save the subsamples
        #rand_idx = time.monotonic()
        file = os.path.join(filepath, '{}_test_samples-{}-ratio_{}-{}.npy'.format(filename, ratio))#, rand_idx))
        np.save(file=file, arr=test_samples)
        file = os.path.join(filepath, 'test_labels{}-ratio_{}-{}.npy'.format(filename, ratio))#, rand_idx))
        np.save(file=file, arr=test_labels)
        file = os.path.join(filepath, 'training_samples-{}-ratio_{}-{}.npy'.format(filename, ratio))#, rand_idx))
        np.save(file=file, arr=training_samples)
        file = os.path.join(filepath, 'training_labels{}-ratio_{}-{}.npy'.format(filename, ratio))#, rand_idx))
